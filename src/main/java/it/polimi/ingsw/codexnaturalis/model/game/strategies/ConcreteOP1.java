...
public class ConcreteOP1 implements Strategy {
    ...
    public int compute(Structure structure) throws IllegalCommandException {
        ...
        for (int i = 40 + radius - 1; i >= 40 - radius + 1; i--) {
            for (int j = 40 - radius + 1 ; j <= 40 + radius -1; j++) {
                ...
                if(upDiago.getSymbol().equals("SHROOM") && !cardToCoordinate.get(upDiago).getVisited() &&
                        centre.getSymbol().equals("SHROOM") && !cardToCoordinate.get(centre).getVisited() &&
                        downDiago.getSymbol().equals("SHROOM") && !cardToCoordinate.get(downDiago).getVisited()){
