...
public class ConcreteOP8 implements Strategy {
    ...
    public int compute(Structure structure) throws IllegalCommandException {
        ...
        for (int i = 40 + radius -1; i >= 40 - radius + 2 ; i--) {
            for (int j = 40 - radius +1; j <= 40 + radius; j++) {
                ...
                if(upBackrest.getSymbol().equals("INSECT") && !cardToCoordinate.get(upBackrest).getVisited() &&
                        lowBackrest.getSymbol().equals("INSECT") && !cardToCoordinate.get(lowBackrest).getVisited() &&
                        seat.getSymbol().equals("ANIMAL") && !cardToCoordinate.get(seat).getVisited()){
