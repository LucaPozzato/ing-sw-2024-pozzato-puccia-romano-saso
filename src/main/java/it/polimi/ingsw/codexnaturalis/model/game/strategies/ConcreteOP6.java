...
public class ConcreteOP6 implements Strategy {
    ...
    public int compute(Structure structure) throws IllegalCommandException {
        ...
        for (int i = 40 + radius -2; i >= 40 - radius +1 ; i--) {
            for (int j = 40 - radius +1; j <= 40 + radius; j++) {
                Card upBackrest = matrix[j][i+2];
                Card lowBackrest = matrix[j][i];
                Card seat = matrix[j-1][i-1];
                if(upBackrest.getSymbol().equals("VEGETABLE") && !cardToCoordinate.get(upBackrest).getVisited() &&
                        lowBackrest.getSymbol().equals("VEGETABLE") && !cardToCoordinate.get(lowBackrest).getVisited() &&
                        seat.getSymbol().equals("INSECT") && !cardToCoordinate.get(seat).getVisited()){
